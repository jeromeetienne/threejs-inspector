 ...
 * @constructor
 */
PanelWin3js.PanelExamples	= function(){
	
	var container	= new UI.Panel()
	
	container.dom.style.overflowY = 'scroll'
	container.dom.style.height = '300px'
	...
	Object.keys(examples).forEach(function(sectionName){
		
		var section = examples[ sectionName ];
		...
		section.forEach(function(basename){
			var url = 'http://threejs.org/examples/'+basename+'.html'

			// build a readableName from this basename
			var readableName = basename.split( '_' );
			readableName.shift();
			...
			item.href = url
			item.textContent = readableName;
			container.dom.appendChild( item )		
