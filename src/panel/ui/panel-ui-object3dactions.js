 ...
 * @constructor
 */
PanelWin3js.PanelObject3DActions	= function(){
...
;(function(){
	...
	function addObjectPerGeometry( parentObject3d, type){
		...
		}else if( type === "TorusKnotGeometry" ){
			var radius = 0.5-0.12;
			var tube = 0.12;
		...
		var parent = InspectedWin3js.getDefaultParent()
		parent.add(mesh)
	}
...
;(function(){
	...
	lightTypes.forEach(function(type){
		...
		container.add(button)
		button.onClick(function(){
			PanelWin3js.functionOnObject3d(addObjectPerLight, [type]);
		})
	})
	
	function addObjectPerLight(parentObject3d, type){
		if( type === "PointLight" ){
			var color = 0xffffff;
			var intensity = 1;
			var distance = 0;
