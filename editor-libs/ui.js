...
UI.CollapsiblePanel = function () {
	...
	this.button.classList.add( 'CollapsiblePanelButton' );
...
UI.CollapsiblePanel.prototype.setCollapsed = function( setCollapsed ) {
	...
	// honor .isCollapsed for this.button
	this.button.classList.remove( 'fa-caret-right' );
	this.button.classList.remove( 'fa-caret-down' );
	this.button.classList.add(this.isCollapsed ? 'fa-caret-right' : 'fa-caret-down' );
};
...
UI.Text.prototype.add = function () {
...
UI.Number = function ( number ) {
	...
	var onMouseMove = function ( event ) {
		...
		scope.value	= Math.min( scope.max, Math.max( scope.min, number ) );
		scope.dom.value	= scope.value.toFixed( scope.precision );
	...
	var onChange = function ( event ) {
		...
		number		= Math.min( scope.max, Math.max( scope.min, number ) );
		number		= isNaN( number ) === false ? number : 0;
		scope.value	= number;
		scope.dom.value	= scope.value.toFixed( scope.precision );
...
UI.Number.prototype.setValue = function ( value ) {
	...
	if ( value !== undefined ) {
		...
		value		= isNaN( value ) === false ? value : 0;
		this.value	= value;
		this.dom.value	= this.value.toFixed( this.precision );
